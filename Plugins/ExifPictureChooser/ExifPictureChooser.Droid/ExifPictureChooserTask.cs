...
namespace KS21.MvvmCross.Plugins.ExifPictureChooser.Droid
{
    public class ExifPictureChooserTask : MvxAndroidTask, IExifPictureChooserTask
    {
        ...
        public void TakePicture(int maxPixelDimension, int percentQuality, Action<Stream, float[]> pictureAvailable,
            Action assumeCancelled)
        {
...
// Specify where to put the image
        ...
        public void ChoosePictureCommon(MvxIntentRequestCode pickId, Intent intent, int maxPixelDimension,
            int percentQuality, Action<Stream, float[]> pictureAvailable, Action assumeCancelled)
        {
...
// Note for furture bug-fixing/maintenance - it might be better to use var outputFileUri = data.GetParcelableArrayExtra("outputFileuri") here?
                ...
                MemoryStream memoryStream = LoadInMemoryBitmap(uri);
                ...
                _currentRequestParameters.PictureAvailable(memoryStream, _latLng);
        ...
        private Bitmap LoadScaledBitmap(Uri uri)
        {
            ...
            int maxDimensionSize = GetMaximumDimension(contentResolver, uri);
...
// - it suggests that Android has returned a corrupt image uri
            ...
            Bitmap sampled = LoadResampledBitmap(contentResolver, uri, sampleSize);
            try
            {
                Bitmap rotated = ExifRotateBitmap(contentResolver, uri, sampled);
                return rotated;
            }
        ...
        private static int GetMaximumDimension(ContentResolver contentResolver, Uri uri)
        {
            using (Stream inputStream = contentResolver.OpenInputStream(uri))
            {
                ...
                Bitmap metadataResult = BitmapFactory.DecodeStream(inputStream, null, optionsJustBounds);
                int maxDimensionSize = Math.Max(optionsJustBounds.OutWidth, optionsJustBounds.OutHeight);
        ...
        private Bitmap ExifRotateBitmap(ContentResolver contentResolver, Uri uri, Bitmap bitmap)
        {
            ...
            //m.elz modivication: get latLng and return the values to callback method
            _latLng = new float[2];
            exif.GetLatLong(_latLng);

            int rotation = exif.GetAttributeInt(ExifInterface.TagOrientation, (Int32) Orientation.Normal);
        ...
        private String GetRealPathFromUri(ContentResolver contentResolver, Uri uri)
        {
            ...
            using (ICursor cursor = contentResolver.Query(uri, proj, null, null, null))
            {
        ...
        private static Int32 ExifToDegrees(Int32 exifOrientation)
        {
            ...
            {
                case (Int32) Orientation.Rotate90:
                    return 90;
                case (Int32) Orientation.Rotate180:
                    return 180;
                case (Int32) Orientation.Rotate270:
                    return 270;
            }
            return 0;
        }
        ...
        {
            public RequestParameters(int maxPixelDimension, int percentQuality, Action<Stream, float[]> pictureAvailable,
                Action assumeCancelled)
            {
            ...
            public Action<Stream, float[]> PictureAvailable { get; private set; }
